class Parser{
	...
	{       Exp exp1, exp2;
		...
		if(tokenCorrente.token == TokenType.NUM)
			if (tokenCorrente.strLexema == null)
				return new Num(Integer.parseInt(String.valueOf(tokenCorrente.lexema)));
			else
				return new Num(Integer.parseInt(tokenCorrente.strLexema));
